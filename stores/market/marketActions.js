...
export function getHoldings(holdings = [], currency ='usd', orderBy = 
'market_cap_desc', sparkline= 'true', priceChangePerc = '7d', perPage=10, page=1) {
    return dispatch => {
        ...
        return axios ({
            ...
            if (response.status == 200) {
                ...
                let myHoldings = response.data.map((item) => {
                    //Retrive our current holdings -> current quantity
...
export function getCoinMarket(currency='usd', orderBy = 
'market_cap_desc', sparkline= true, priceChangePerc= '7d',
 perPage=10, page=1) {
    return dispatch => {
